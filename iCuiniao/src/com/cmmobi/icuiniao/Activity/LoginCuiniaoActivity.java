 ...
 *
 */
public class LoginCuiniaoActivity extends Activity implements AnimationListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(UserUtil.userid != -1&&UserUtil.userState == 1){
			...
			loginpage = (ScrollView)findViewById(R.id.scrollview);
			...
			//获取用户头像
			mConnectUtil = new ConnectUtil(LoginCuiniaoActivity.this, mHandler,1,0);
			mConnectUtil.connect(URLUtil.URL_GET_USERICON+UserUtil.userid+".jpg", HttpThread.TYPE_IMAGE, 0);
			addProgress();
		}else{
			type = TYPE_LOGIN;
			setContentView(R.layout.logincuiniao);
			loginpage = (ScrollView)findViewById(R.id.scrollview);
			titlebar_menuLeft = (Button)findViewById(R.id.titlebar_leftmenu);
			mainMenu = (MainMenu)findViewById(R.id.mainmenu);
			titlebar_menubutton = (Button)findViewById(R.id.titlebar_menubutton);
				...
				public boolean onTouch(View v, MotionEvent event) {				
					...
					case MotionEvent.ACTION_UP:
						...
						if(moveX < 15 * ratio && moveY < 15 * ratio){
							CommonUtil.hideSoftInput(input_name, LoginCuiniaoActivity.this);
						}
						break;	
					}							
					return false;
	...
	private OnClickListener regeditClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			// TODO Auto-generated method stub
			startActivityForResult(new Intent(LoginCuiniaoActivity.this, RegeditActivity.class),9001);
		}
	};
	...
	private OnClickListener loginClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(input_name.getText() != null&&input_name.getText().length() > 0){
				...
				CommonUtil.ShowToast(LoginCuiniaoActivity.this, "请输入常用邮箱");
				return;
			}
			if(input_password.getText() != null&&input_password.getText().length() > 0){
				pw = input_password.getText().toString().trim();
			}else{
				CommonUtil.ShowToast(LoginCuiniaoActivity.this, "请输入密码");
				return;
			}
			mConnectUtil = new ConnectUtil(LoginCuiniaoActivity.this, mHandler,0);
			mConnectUtil.connect(addUrlParam(URLUtil.URL_LOGIN, name, pw, 0), HttpThread.TYPE_PAGE, 0);
			if(CommonUtil.isNetWorkOpen(LoginCuiniaoActivity.this)){
				Intent intent11 = new Intent();
				intent11.setClass(LoginCuiniaoActivity.this, WebViewLoginActivity.class);
				String url = addUrlParam(URLUtil.URL_LOGIN, name, pw, 0);
				LogPrint.Print("webview","login url = "+url);
				intent11.putExtra("url", url);
				intent11.putExtra("logintype", 0);  //用于判断是翠鸟登录
				startActivityForResult(intent11, 9000);
			}else{
				CommonUtil.ShowToast(LoginCuiniaoActivity.this, "杯具了- -!\n联网不给力啊");
			}
	...
	private OnClickListener findpwClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			startActivity(new Intent(LoginCuiniaoActivity.this, FindPasswordActivity.class));
		}
	};
					...
					if(result.equalsIgnoreCase("true")){
						...
						try {
							...
							if(oid != null){
								...
								CommonUtil.saveUserId(LoginCuiniaoActivity.this, UserUtil.userid);
							}
						} catch (Exception e) {
							// TODO: handle exception
						}
						CommonUtil.saveUserState(LoginCuiniaoActivity.this, UserUtil.userState);
						CommonUtil.saveAddrManager(LoginCuiniaoActivity.this, "");
						CommonUtil.saveCurAddr(LoginCuiniaoActivity.this, -1);
						CommonUtil.ShowToast(LoginCuiniaoActivity.this, "退出成功，小C会想你的。");
						//清空设置信息
						//清空通知提醒
						CommonUtil.saveMessageReceiverState(LoginCuiniaoActivity.this, true);
						//清空所在地
						CommonUtil.saveArea(LoginCuiniaoActivity.this, "");
						//清空好友可见
						CommonUtil.saveLikeFriendState(LoginCuiniaoActivity.this, false);
						//清空分享公开
						CommonUtil.saveShareFriendState(LoginCuiniaoActivity.this, false);
						//清空公开评论
						CommonUtil.saveBaskFriendState(LoginCuiniaoActivity.this, false);
						//清空允许任何人加好友
						CommonUtil.saveAllAddFriendState(LoginCuiniaoActivity.this, false);
						//清空真实姓名
						CommonUtil.saveRealName(LoginCuiniaoActivity.this, "");
						//清空生日
						CommonUtil.saveBirthday(LoginCuiniaoActivity.this, "");
						//清空兴趣
						CommonUtil.saveInterest(LoginCuiniaoActivity.this, "");
						//清空职业
						CommonUtil.saveOccuption(LoginCuiniaoActivity.this, "");
						//清空月收入
						CommonUtil.saveMoney(LoginCuiniaoActivity.this, "");
						//清空教育程度
						CommonUtil.saveEducation(LoginCuiniaoActivity.this, "");
						//清空婚姻状况
						CommonUtil.saveMarry(LoginCuiniaoActivity.this, "");
						
						CommonUtil.saveCoqutryButtonState(LoginCuiniaoActivity.this, false);
						//停止信息服务
						stopService(new Intent(LoginCuiniaoActivity.this,IMService.class));
						OfflineLog.writeMainMenu_MessageButton((byte)0);//写入离线日志
						Intent intent11 = new Intent();
						intent11.setClass(LoginCuiniaoActivity.this, LoginAndRegeditActivity.class);
						startActivity(intent11);
						//发送登出广播
						sendBroadcast(new Intent(ActionID.ACTION_BROADCAST_LOGOUT));
						//重新连接
						startService(new Intent(LoginCuiniaoActivity.this,IMService.class));
						finish();
					}else{
						CommonUtil.ShowToast(LoginCuiniaoActivity.this, "退出失败，爱你就应给你自由，在退一次");
					}
				}
			}else{//登录
//				if(result != null){
