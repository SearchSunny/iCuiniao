...
//		setContentView(mPageLayout);
	...
	public void init(){
		...
		case 0://关注
			if(urlString != null){
				mConnectUtil.connect(addUrlParam(urlString, UserUtil.userid, 0, msgString, "addover"), HttpThread.TYPE_PAGE,0);
			}
			break;
		case 1://相互关注
			if(urlString != null){
				mConnectUtil.connect(addUrlParam(urlString, UserUtil.userid, 0, msgString, "addeachother"), HttpThread.TYPE_PAGE,0);
			}
			break;
		case 2://粉丝
			if(urlString != null){
				mConnectUtil.connect(addUrlParam(urlString, UserUtil.userid, 0, msgString, "add"), HttpThread.TYPE_PAGE,0);
			}
			break;
		}
	}
	...
	public void init(String urlString,String msg){
		...
		case 0://关注
			if(urlString != null){
				mConnectUtil.connect(addUrlParam(urlString, UserUtil.userid, 0, msg, "addover"), HttpThread.TYPE_PAGE,0);
			}
			break;
		case 1://相互关注
			if(urlString != null){
				mConnectUtil.connect(addUrlParam(urlString, UserUtil.userid, 0, msg, "addeachother"), HttpThread.TYPE_PAGE,0);
			}
			break;
		case 2://粉丝
			if(urlString != null){
				mConnectUtil.connect(addUrlParam(urlString, UserUtil.userid, 0, msg, "add"), HttpThread.TYPE_PAGE,0);
			}
			break;
		}
	}
			...
			case MessageID.MESSAGE_CONNECT_LAYOUTOVER:
				...
				if(downImageItem != null){
					...
					if(parserEngine.getPageObject().getPageId() == downImageItem.getPageId()){
						...
						if(urlString != null){
							new ConnectUtil(SearchFriendActivity.this, mHandler,0).connect(urlString, HttpThread.TYPE_IMAGE, downImageItem.getIndex());
						}
					}
					//发起下一个询问
				...
				if(isMoreItem){//查看更多
					if(curIndex < totalpage){
						if(URLUtil.IsLocalUrl()){
							...
							switch (tabbuttonIndex) {
							case 0://关注
								new ConnectUtil(SearchFriendActivity.this, mHandler,0).connect(addUrlParam(URLUtil.URL_SEARCHFRIEND_MORE_LISTITEM, UserUtil.userid, curIndex, msgString, "addover"), HttpThread.TYPE_PAGE,0);
								break;
							case 1://相互关注
								new ConnectUtil(SearchFriendActivity.this, mHandler,0).connect(addUrlParam(URLUtil.URL_SEARCHFRIEND_MORE_LISTITEM, UserUtil.userid, curIndex, msgString, "addeachother"), HttpThread.TYPE_PAGE,0);
								break;
							case 2://粉丝
								new ConnectUtil(SearchFriendActivity.this, mHandler,0).connect(addUrlParam(URLUtil.URL_SEARCHFRIEND_MORE_LISTITEM, UserUtil.userid, curIndex, msgString, "add"), HttpThread.TYPE_PAGE,0);
								break;
							}
						}
					}
					...
					intent.setClass(SearchFriendActivity.this, UserPageActivity.class);
...
//				if(url1 != null){
//					new ConnectUtil(SearchFriendActivity.this, mHandler).connect(editUrl(url1, str), HttpThread.TYPE_PAGE, 0);
//				}
				if(str.length() > 0){
					Intent intent = new Intent();
					...
					intent.setClass(SearchFriendActivity.this, SearchFriendActivity.class);
					startActivity(intent);
					finish();
				}
				break;
...
//		}
	...
	public String addUrlParam(String url,int oid,int pi,String msg,String keyword){
		if(URLUtil.IsLocalUrl()){
			return url;
		}
		if(msg == null)msg = "";
		return url+"?oid="+oid+"&pi="+pi+"&msg="+CommonUtil.toUrlEncode(msg)+"&keyword="+keyword;
	}
}
