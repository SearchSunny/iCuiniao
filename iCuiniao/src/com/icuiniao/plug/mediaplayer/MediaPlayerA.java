 ...
 *
 */
public class MediaPlayerA extends Activity implements SurfaceHolder.Callback{
		...
		builder.setMessage(str).setPositiveButton("登录",
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						CommonUtil.ShowToast(MediaPlayerA.this,
								"请登陆或者注册，变身为小C的主人！");
						mHandler.removeCallbacks(cloassRunnable);
						pauseing = true;
						Intent intent11 = new Intent();
						intent11.setClass(MediaPlayerA.this,
								LoginAndRegeditActivity.class);						
						MediaPlayerA.this.startActivity(intent11);						
	...
	private OnClickListener lookClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(wapUrl != null){
				...
				intent.setClass(MediaPlayerA.this, WebviewActivity.class);
				...
				startActivityForResult(intent, MessageID.REQUESTCODE_LIKE_FLUSH);
			}
		}
	};
	...
	private android.media.MediaPlayer.OnPreparedListener onPreparedListener = new android.media.MediaPlayer.OnPreparedListener(){
		...
		public void onPrepared(android.media.MediaPlayer mp) {
			...
			if(isSurfaceDestory){
				...
				isScreen = true;
				flag = true;
				mHandler.sendEmptyMessageDelayed(MessageID.MESSAGE_MEDIA_CLOSE, 3000);
			}else{
				mediaPlayer.start();
				changePlayRes(true);
				setCurTime(0);
...
//				mHandler.sendEmptyMessageDelayed(MessageID.MESSAGE_MEDIA_CURRENTTIME, 1000);
	...
	private OnClickListener shareListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.buttonWB:
				...
				intent.setClass(MediaPlayerA.this, ShareEditActivity.class);
				startActivity(intent);
			...
			case R.id.buttonQQ:
				...
				qqIntent.setClass(MediaPlayerA.this, ShareEditActivity.class);
				startActivity(qqIntent);
				OfflineLog.writeShareQQ(commodityid);
				break;
			case R.id.buttonWX:
				LogPrint.Print("shareItem = wx");
				if(CommonUtil.isNetWorkOpen(MediaPlayerA.this)){
				// 通过WXAPIFactory工厂，获取IWXAPI的实例
				api = WXAPIFactory.createWXAPI(MediaPlayerA.this, WXAppID.APP_ID, false);
				//注册微信
				api.registerApp(WXAppID.APP_ID);
					//检查是否安装微信
			    	if(!api.isWXAppInstalled()){
			    		//提示用户安装微信
			    		Toast.makeText(MediaPlayerA.this, "未检测到微信终端", Toast.LENGTH_SHORT).show();
			    	}else{
						//判断缓存是否有图片
						if(CommonUtil.exists(CommonUtil.dir_cache+"/"+CommonUtil.urlToNum(commodityImageString))){		
							...
							new ConnectUtil(MediaPlayerA.this, mHandler, 0).connect(commodityImageString, HttpThread.TYPE_IMAGE, 10);
						}
			    	}
				}else{	
					CommonUtil.ShowToast(MediaPlayerA.this, "杯具了- -!\n联网不给力啊");
				}
				break;
			case R.id.buttonEmail:
				LogPrint.Print("shareItem = email");
				new ConnectUtil(MediaPlayerA.this, mHandler, 0).connect(URLUtil.URL_GET_SHORTLINK+"?type=5"+"&oid="+UserUtil.userid+"&cid="+commodityid, HttpThread.TYPE_PAGE, 1);
				new ConnectUtil(MediaPlayerA.this, mHandler, 0).connect(URLUtil.URL_SHARE_UPLAOD+"?logintype=11"+"&oid="+UserUtil.userid+"&cid="+commodityid, HttpThread.TYPE_PAGE, 0);
				break;
			case R.id.buttonSms:
				LogPrint.Print("shareItem = sms");
				new ConnectUtil(MediaPlayerA.this, mHandler, 0).connect(URLUtil.URL_GET_SHORTLINK+"?type=6"+"&oid="+UserUtil.userid+"&cid="+commodityid, HttpThread.TYPE_PAGE, 0);
				new ConnectUtil(MediaPlayerA.this, mHandler, 0).connect(URLUtil.URL_SHARE_UPLAOD+"?logintype=12"+"&oid="+UserUtil.userid+"&cid="+commodityid, HttpThread.TYPE_PAGE, 0);
				break;
			case R.id.buttonPengyou:
	...
	public void shareEmail(String title,String message,String imgDir){
		try {
			...
			CommonUtil.ShowToast(MediaPlayerA.this, "主人，邮件罢工了，呼叫失败！");
		}
	}
...
//					}
	...
	public boolean isWeiXinFreind(){
		...
		if (wxSdkVersion >= TIMELINE_SUPPORTED_VERSION) {
			...
			CommonUtil.ShowToast(MediaPlayerA.this, "此版本不支持!");
			return false;
		}
	}
}
